Remove-Item C:\temp\mail.html
New-Item -ItemType file -Path C:\temp\mail.html
$mailboxCountThreshold =  30
$dbSizeThreshold = 10GB
$backupThreshold = 1
$mbxSizeThreshold = 30MB
$fileName = "C:\temp\mail.html"
...
Add-Content $fileName '<title>Database Report</title>'
...
Function get-DBInfo
{
 $dbs = Get-MailboxServer |Get-MailboxDatabase -Status
 foreach($db in $dbs)
 {
 	$name = $db.name
	$svr = $db.servername	
