<#
    ...
    Creates a HTML Report describing the Exchange environment 
   	...
   	Steve Goodman
    (Updated by Neil Johnson to support Exchange Server 2013)
...
function _GetOverview
{
	...
	#$Output+="<col width=""4%""><col width=""4%""><col width=""3%""><col width=""3%""><col width=""3%""><col width=""3%"">";
	...
	<th colspan=""$($ExchangeEnvironment.TotalServersByRole.Count)""><font color=""#ffffff"">Roles Installed</font></th>
	<th><font color=""#ffffff"">OS Version</font></th>
	<th><font color=""#ffffff"">OS Service Pack</font></th></tr>"
	$TotalMailboxes=0
	$Servers.Value | %{$TotalMailboxes += $_.Mailboxes}
	$Output+="<tr bgcolor=""$($BGColSubHeader)""><th><font color=""#ffffff"">Mailboxes: $($TotalMailboxes)</font></th><th></th>"
	...
	foreach ($Server in $Servers.Value)
	{
		...
		$Output+="</td>
		<td>$($ExVersionStrings["$($Server.ExchangeMajorVersion).$($Server.ExchangeSPLevel)"].Long)"
...
<th colspan=""$($ExchangeEnvironment.TotalMailboxesByVersion.Count+1)""><font color=""#ffffff"">Total Mailboxes:</font></th>
<th colspan=""$($ExchangeEnvironment.TotalServersByRole.Count)""><font color=""#ffffff"">Total Roles:</font></th></tr>
<tr bgcolor=""#00CC00"">"
...
$dbcopy =Get-MailboxDatabase |Get-MailboxDatabaseCopyStatus

$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""30%""><col width=""20%""><col width=""10%""><col width=""10%""><col width=""30%"">
    	 <tr align=""left"" bgcolor=""#ADD8E6""><th>Mailbox Databases Copies</th><th>Status</th><th>Copy Queue</th><th>Replay Queue</th><th>Content Index Status</th></table>"
$Output+="<table border=""0"" cellpadding=""3"" width=""100%"" style=""font-size:8pt;font-family:Arial,sans-serif"">
<col width=""30%""><col width=""20%""><col width=""10%""><col width=""10%""><col width=""30%""><tr align=""left"">"

$dbcopy =Get-MailboxDatabase |Get-MailboxDatabaseCopyStatus
$AlternateRow=0;
foreach($copy in $dbcopy){



$Output+="<tr"
		if ($AlternateRow -eq 1)
		{
			$Output+=" style=""background-color:#dddddd"""
			$AlternateRow=0
		} else
...
[String]$DBName=$copy.name
[String]$DBStatus=$copy.status
[String]$DBCopyStatus=$copy.copyqueuelength
[String]$DBReplayStatus =$copy.replayqueuelength
[String]$DBIndex=$copy.ContentindexState
if ($dbStatus -eq "Suspended" -or $dbstatus -eq "Failed" )
{
	$Output+="><td>$DBName</td><td bgcolor =""#FF0000"">$DBStatus</td><td>$DBCopyStatus</td><td>$dbReplayStatus</td>"
}else
{
$Output+="><td>$DBName</td><td>$DBStatus</td><td>$DBCopyStatus</td><td>$dbReplayStatus</td>"
}

if ($DBIndex -eq "Suspended" -or $DBIndex -eq "Failed")
{
	$Output+="<td bgcolor =""#FF0000"">$DbIndex</td></tr>"
}else
{
	$Output+="<td>$DbIndex</td></tr>"
}
